...
/* ---- Avatar rendern (Profilbild oder Initialen) ---- */
export function renderAvatar(username, avatarUrl, size = 'md') {
  ...
  const fontSizes = { sm: '0.65rem', md: '0.8rem', lg: '1rem' };
  ...
  if (avatarUrl) {
    return `<div style="${base}border:1.5px solid var(--border);">
      <img src="${avatarUrl}" alt="${initials}"
           style="width:100%;height:100%;object-fit:cover;">
    </div>`;
  }

  return `<div style="${base}background:var(--accent);color:white;">
    ${initials}
  </div>`;
}
